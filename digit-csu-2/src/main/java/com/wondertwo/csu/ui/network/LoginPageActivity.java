...
import com.wondertwo.csu.utils.NetworkStateUtils;
...
public class LoginPageActivity extends BaseNetworkActivity {
    ...
    public void onLoginButtonClicked(View view) {
        ...
        if (TextUtils.isEmpty(userNumber)) {
             ...
             * 判断WIFI是否连接
             */
            if (!NetworkStateUtils.checkWifiConnection(this)) {
                /**
                 * WIFI未连接，弹出提示连接框
                 */
    ...
    private void performForLogin() {
        ...
        AsyncTaskWarpper.getATWInstance().doAsyncWork(new Callable<Object>() {
            ...
            public void onNotify(Object result) {
                ShowingToastUtils.showToastLong(LoginPageActivity.this, "取消");
            }
        }, new NotifyListener() {
            //登陆后的回调
            @Override
                    ...
                    try {
                        ...
                        if (jsonObject.has("resultCode") && (resultCode = jsonObject.getInt("resultCode")) < arr.length) {
                            switch (resultCode) {
                                ...
                                case 0:
                                case 10:
                                    ShowingToastUtils.showToastLong(LoginPageActivity.this, R.string.login_success);
                ...
                if (!TextUtils.isEmpty(ar.getArgs()[0].toString())) {
                    try {
                        ...
                        if (jsonObject.has("resultCode") && (resultCode = jsonObject.getInt("resultCode")) < arr.length) {
                            switch (resultCode) {
                                case 0:
                                    ShowingToastUtils.showToastLong(LoginPageActivity.this, R.string.logout_success);
                                    break;
                                default:
                                    showError(arr[resultCode]);
                            }
    ...
    private void doTextViewEmptyTip(EditText et) {
        ...
        ShowingToastUtils.showToastLong(this, getResources().getString(R.string.textview_empty));
    }
}
